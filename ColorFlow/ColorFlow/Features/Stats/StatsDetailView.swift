...
struct StatsDetailView: View {
    ...
    private var xpSection: some View {
        ...
        return VStack(spacing: 10) {
            ...
            if level < GameConfig.xpPerLevel.count {
                Text("\(GameConfig.xpPerLevel[level] - xp) XP to next level")
                    .font(.caption)
                    .mutedText()
            }
        }
    ...
    private func worldsUnlocked() -> Int {
        ...
        return World.all.filter { world in
            world.levelRange.allSatisfy { best[$0] != nil }
        }.count
    }
}
