...
struct HomeView: View {
    ...
    private var currentWorld: World { World.worldForLevel(nextLevel) }
    ...
    private var xpBadge: some View {
        ...
        let level = GameConfig.levelForXP(xp)
        let progress = GameConfig.xpProgressInLevel(xp)
        let title = GameConfig.xpLevelTitle(level)
    ...
    private var playHero: some View {
        NavigationLink(value: Route.game(mode: .level(index: nextLevel))) {
            VStack(spacing: 14) {
                ZStack {
                    Circle()
                ...
                VStack(spacing: 4) {
                    if nextLevel > 0 {
                        Text("\(currentWorld.name) — Level \(World.localIndex(forLevel: nextLevel) + 1)")
                    ...
                    Text("Level \(nextLevel + 1) of \(GameConfig.totalLevels)")
            ...
            .background {
                RoundedRectangle(cornerRadius: 22)
                    .fill(
                        LinearGradient(
                            colors: [theme.colors.accent.opacity(0.08), theme.colors.accent.opacity(0.02)],
                            startPoint: .top,
                            endPoint: .bottom
                        )
                    )
    ...
    private var modeButtons: some View {
        HStack(spacing: 12) {
            NavigationLink(value: Route.game(mode: .daily)) {
                modeCard(icon: "calendar", title: "Daily", color: .orange)
            }
            .buttonStyle(BounceButtonStyle())

            NavigationLink(value: Route.game(mode: .infinite)) {
                modeCard(icon: "infinity", title: "Quick Play", color: theme.colors.secondary)
            }
            .buttonStyle(BounceButtonStyle())
        }
    }
    ...
    private var quickNav: some View {
        HStack(spacing: 10) {
            NavigationLink(value: Route.worldMap) {
    ...
    private var statsSection: some View {
        HStack(spacing: 0) {
            gameStat(value: "\(currentStats?.puzzlesCompleted ?? 0)", label: "Cleared", icon: "bolt.fill", color: theme.colors.accent)
            statDivider
...
struct FloatingOrbsView: View {
    ...
    private func orbColor(_ i: Int) -> Color {
        [theme.colors.accent, theme.colors.secondary, theme.colors.primary,
         .orange, .purple, theme.colors.accent][i % 6]
    }
