...
final class SoundService {
    ...
    func playDiscover() { play("discover") }
    func playFogClear() { play("fog") }
    ...
    private func generateSounds() {
        ...
        buffers["discover"] = synth(0.25) { t in
            let freq: Float = 440 + 220 * t / 0.25
            let env: Float = t < 0.01 ? t / 0.01 : expf(-(t - 0.01) * 6)
            return sinf(2 * .pi * freq * t) * env * 0.3
        }
        buffers["fog"] = synth(0.3) { t in
            let env: Float = t < 0.02 ? t / 0.02 : (0.3 - t) / 0.28
            return sinf(2 * .pi * 330 * t) * max(0, env) * 0.2
        }
    }
