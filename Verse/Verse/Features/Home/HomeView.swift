...
struct HomeView: View {
    ...
    private var currentChapter: Chapter { Chapter.chapterForLevel(nextLevel) }
    ...
    var body: some View {
        ...
        .toolbar {
            ToolbarItem(placement: .principal) {
                Text("Verse").font(.system(size: 28, weight: .black, design: .serif)).primaryText()
            }
    ...
    private var xpBadge: some View {
        ...
        let level = VerseConfig.levelForXP(xp)
        let progress = VerseConfig.xpProgressInLevel(xp)
        let title = VerseConfig.xpLevelTitle(level)
    ...
    private var playHero: some View {
        NavigationLink(value: Route.verse(mode: .level(index: nextLevel))) {
            VStack(spacing: 14) {
                ZStack {
                    Circle().fill(theme.colors.accent.opacity(0.15)).frame(width: 100, height: 100)
                        .scaleEffect(playButtonPulse ? 1.15 : 1.0).blur(radius: 8)
                    Circle()
                        ...
                        .overlay {
                            Image(systemName: "pencil.line").font(.system(size: 28, weight: .bold)).foregroundStyle(.white)
                        }
                }
                VStack(spacing: 4) {
                    if nextLevel > 0 {
                        Text("\(currentChapter.name) — Poem \(Chapter.localIndex(forLevel: nextLevel) + 1)")
                            .font(.system(size: 16, weight: .bold, design: .rounded)).primaryText()
                    } else {
                        Text("Compose a Verse").font(.system(size: 16, weight: .bold, design: .rounded)).primaryText()
                    }
                    Text("Poem \(nextLevel + 1) of \(VerseConfig.totalLevels)")
        ...
        return HStack(spacing: 10) {
            ZStack {
                ...
                Image(systemName: streak > 0 ? "flame.fill" : "flame")
                    .font(.system(size: 22)).foregroundStyle(streak > 0 ? .orange : theme.colors.textMuted)
            }
            VStack(alignment: .leading, spacing: 2) {
                Text(streak > 0 ? "\(streak)-day streak!" : "Start your streak!")
                    .font(.system(size: 15, weight: .bold, design: .rounded)).primaryText()
                Text(streak > 0 ? "Write today to keep it going" : "Write daily to build a streak")
    ...
    private var modeButtons: some View {
        HStack(spacing: 12) {
            NavigationLink(value: Route.verse(mode: .daily)) {
                modeCard(icon: "calendar", title: "Daily", color: .orange)
            }.buttonStyle(BounceButtonStyle())
            NavigationLink(value: Route.verse(mode: .freePlay)) {
                modeCard(icon: "infinity", title: "Free Write", color: theme.colors.secondary)
            }.buttonStyle(BounceButtonStyle())
        }
    }
    ...
    private var quickNav: some View {
        HStack(spacing: 10) {
            NavigationLink(value: Route.chapterMap) {
                navButton(icon: "book.fill", label: "Chapters", color: Color(hex: "6C63FF"))
            }.buttonStyle(BounceButtonStyle())
    ...
    private var statsSection: some View {
        HStack(spacing: 0) {
            gameStat(value: "\(currentStats?.poemsCompleted ?? 0)", label: "Written", icon: "pencil.line", color: theme.colors.accent)
            statDivider
...
struct FloatingOrbsView: View {
    ...
    private func orbColor(_ i: Int) -> Color {
        [theme.colors.accent, theme.colors.secondary, theme.colors.primary, .orange, .purple, theme.colors.accent][i % 6]
    }
