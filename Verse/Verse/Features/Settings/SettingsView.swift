...
struct SettingsView: View {
    ...
    private var gameSection: some View {
        Section("Game") {
            Toggle("Show Timer", isOn: showTimerBinding).listRowBackground(theme.colors.surface.opacity(0.3))
        }
    }
        ...
        Section("Appearance") {
            NavigationLink(value: Route.themePicker) {
                HStack {
                    Text("Theme")
                    Spacer()
                    Text(Themes.theme(for: currentSettings?.currentTheme ?? "parchment").name).foregroundStyle(theme.colors.textSecondary)
                }
            }
    ...
    private var showTimerBinding: Binding<Bool> {
        Binding(get: { currentSettings?.showTimer ?? true }, set: { currentSettings?.showTimer = $0; HapticsService.selection() })
    }

    private var soundBinding: Binding<Bool> {
        Binding(get: { currentSettings?.soundEffects ?? true }, set: { currentSettings?.soundEffects = $0; SoundService.shared.configure(enabled: $0); HapticsService.selection() })
    }

    private var hapticsBinding: Binding<Bool> {
        Binding(get: { currentSettings?.haptics ?? true }, set: { currentSettings?.haptics = $0; HapticsService.configure(enabled: $0); HapticsService.selection() })
    }
            ...
            set: { newValue in
                ...
                if newValue {
                    Task {
                        ...
                        if granted { NotificationService.scheduleDailyReminder(hour: currentSettings?.dailyReminderHour ?? 19, minute: currentSettings?.dailyReminderMinute ?? 0) }
                    }
    ...
    private func resetProgress() {
        if let s = currentSettings { s.currentLevelIndex = 0; s.highestUnlockedLevel = 0 }
