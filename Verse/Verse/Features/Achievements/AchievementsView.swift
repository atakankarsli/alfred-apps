...
struct AchievementsView: View {
    ...
    private func achievementCard(_ achievement: Achievement) -> some View {
        ...
        return VStack(spacing: 6) {
            ZStack {
                Circle()
                    .fill(isUnlocked ? tierColor(achievement.tier).opacity(0.15) : theme.colors.cellBackground.opacity(0.4))
            ...
            Text(showDetails ? achievement.title : "???").font(.system(size: 11, weight: .semibold, design: .rounded)).foregroundStyle(isUnlocked ? theme.colors.text : theme.colors.textMuted).lineLimit(1)
            if showDetails {
        ...
        .background {
            RoundedRectangle(cornerRadius: 12)
                .fill(isUnlocked ? tierColor(achievement.tier).opacity(0.05) : theme.colors.surface.opacity(0.2))
                .overlay {
                    RoundedRectangle(cornerRadius: 12).strokeBorder(isUnlocked ? tierColor(achievement.tier).opacity(0.2) : theme.colors.boardBorder.opacity(0.08), lineWidth: 1)
                }
        }
    }
