...
struct StatsDetailView: View {
    ...
    private var xpSection: some View {
        ...
        return VStack(spacing: 10) {
            ...
            if level < VerseConfig.xpPerLevel.count {
                Text("\(VerseConfig.xpPerLevel[level] - xp) XP to next level").font(.caption).mutedText()
            }
        }
        .padding(16)
        .background {
    ...
    private var poemStats: some View {
        statsCard(title: "Poetry", rows: [
            ("Poems Completed", "\(s?.poemsCompleted ?? 0)"),
            ("3-Star Poems", "\(s?.threeStarCount ?? 0)"),
            ("Time Spent Writing", formatTime(s?.totalTimePlayed ?? 0)),
    ...
    private var streakStats: some View {
        statsCard(title: "Streaks", rows: [
            ...
            ("Daily Poems", "\(s?.dailyPoemsCompleted ?? 0)"),
        ])
    }
