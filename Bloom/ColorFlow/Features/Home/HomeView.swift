...
struct HomeView: View {
    ...
    private var nextGarden: Int { currentSettings?.currentLevelIndex ?? 0 }
    private var currentSeason: Season { Season.seasonForGarden(nextGarden) }
    ...
    private var xpBadge: some View {
        ...
        return HStack(spacing: 14) {
            ZStack {
                Circle()
                    .fill(
                        RadialGradient(
                            colors: [Color(hex: "6BCB77"), Color(hex: "6BCB77").opacity(0.3)],
                            center: .center,
                            startRadius: 0,
                            endRadius: 28
                        )
                ...
                GeometryReader { geo in
                    ZStack(alignment: .leading) {
                        ...
                        Capsule()
                            .fill(
                                LinearGradient(
                                    colors: [Color(hex: "6BCB77"), Color(hex: "6BCB77").opacity(0.7)],
                                    startPoint: .leading,
                                    endPoint: .trailing
                                )
                            )
                            .frame(width: max(10, geo.size.width * progress), height: 10)
                            .shadow(color: Color(hex: "6BCB77").opacity(0.5), radius: 4)
                    }
                }
                .frame(height: 10)
            }
        ...
        .background {
            RoundedRectangle(cornerRadius: 18)
                ...
                .overlay {
                    RoundedRectangle(cornerRadius: 18)
                        .strokeBorder(Color(hex: "6BCB77").opacity(0.2), lineWidth: 1)
                }
        }
    }
    ...
    private var playHero: some View {
        NavigationLink(value: Route.garden(mode: .garden(index: nextGarden))) {
            VStack(spacing: 14) {
                ZStack {
                    Circle()
                    ...
                    Circle()
                        ...
                        .shadow(color: Color(hex: "6BCB77").opacity(0.5), radius: 12, y: 4)
                        .overlay {
                ...
                VStack(spacing: 4) {
                    if nextGarden > 0 {
                        Text("\(currentSeason.name) — Garden \(Season.localIndex(forGarden: nextGarden) + 1)")
                    ...
                    Text("Garden \(nextGarden + 1) of \(BloomConfig.totalGardens)")
            ...
            .background {
                RoundedRectangle(cornerRadius: 22)
                    .fill(
                        LinearGradient(
                            colors: [Color(hex: "6BCB77").opacity(0.08), Color(hex: "6BCB77").opacity(0.02)],
                            startPoint: .top,
                            endPoint: .bottom
                        )
                    )
        ...
        return HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(streak > 0 ? "\(streak)-day streak!" : "Start your streak!")
                    ...
                    .primaryText()
                Text(streak > 0 ? "Tend your garden today" : "Plant daily to grow a streak")
                    .font(.system(size: 12))
                    .secondaryText()
            }
    ...
    private var modeButtons: some View {
        HStack(spacing: 12) {
            NavigationLink(value: Route.garden(mode: .daily)) {
                modeCard(icon: "calendar", title: "Daily", color: .orange)
            }
            .buttonStyle(BounceButtonStyle())

            NavigationLink(value: Route.garden(mode: .freeGarden)) {
                modeCard(icon: "leaf.circle", title: "Free Plant", color: Color(hex: "6BCB77"))
            }
            .buttonStyle(BounceButtonStyle())
        }
    }
    ...
    private var quickNav: some View {
        HStack(spacing: 10) {
            NavigationLink(value: Route.seasonMap) {
                gameNavButton(icon: "map.fill", label: "Seasons", color: Color(hex: "6BCB77"))
            }
            .buttonStyle(BounceButtonStyle())
    ...
    private func gameNavButton(icon: String, label: String, color: Color) -> some View {
        ...
        .background {
            RoundedRectangle(cornerRadius: 16)
                ...
                .overlay {
                    RoundedRectangle(cornerRadius: 16)
                        .strokeBorder(theme.colors.boardBorder.opacity(0.08), lineWidth: 1)
                }
        }
    }
    ...
    private var statsSection: some View {
        HStack(spacing: 0) {
            gameStat(value: "\(currentStats?.puzzlesCompleted ?? 0)", label: "Gardens", icon: "leaf.fill", color: Color(hex: "6BCB77"))
            statDivider
...
struct FloatingOrbsView: View {
    ...
    private func orbColor(_ i: Int) -> Color {
        [Color(hex: "6BCB77"), Color(hex: "FFD93D"), Color(hex: "FF6B6B"),
         Color(hex: "5BC0EB"), Color(hex: "9B5DE5"), Color(hex: "FF8C42")][i % 6]
    }
