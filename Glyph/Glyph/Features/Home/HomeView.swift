...
struct HomeView: View {
    ...
    private var currentCategory: SymbolCategory { SymbolCategory.categoryForChallenge(nextLevel) }
    ...
    private var drawHero: some View {
        NavigationLink(value: Route.draw(index: nextLevel)) {
            VStack(spacing: 14) {
                ZStack {
                    Circle()
                        .fill(theme.colors.accent.opacity(0.15))
                        .frame(width: 100, height: 100)
                        .scaleEffect(drawPulse ? 1.15 : 1.0)
                        .blur(radius: 8)
                    Circle()
                ...
                VStack(spacing: 4) {
                    if nextLevel > 0 {
                        Text("\(currentCategory.displayName) — Symbol \(SymbolCategory.localIndex(forChallenge: nextLevel) + 1)")
                    ...
                    Text("Symbol \(nextLevel + 1) of \(GlyphConfig.totalChallenges)")
    ...
    private var streakBanner: some View {
        ...
        return HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text(streak > 0 ? "Draw today to keep it going" : "Draw daily to build a streak")
    ...
    private var modeButtons: some View {
        HStack(spacing: 12) {
            ...
            NavigationLink(value: Route.gallery) {
                modeCard(icon: "photo.on.rectangle.angled", title: "Gallery", color: theme.colors.secondary)
            }.buttonStyle(BounceButtonStyle())
        }
    }
    ...
    private var quickNav: some View {
        HStack(spacing: 10) {
            NavigationLink(value: Route.worlds) {
                navButton(icon: "map.fill", label: "Worlds", color: Color(hex: "FF6B6B"))
            }.buttonStyle(BounceButtonStyle())
    ...
    private var statsSection: some View {
        HStack(spacing: 0) {
            statItem(value: "\(currentStats?.symbolsCompleted ?? 0)", label: "Drawn", icon: "pencil.tip", color: theme.colors.accent)
            statDivider
