...
final class StatsRecord {
    ...
    var categoriesUsedData: Data?

    init(
        symbolsCompleted: Int = 0,
        ...
        categoriesUsedData: Data? = nil
    ) {
        self.symbolsCompleted = symbolsCompleted
        ...
        self.categoriesUsedData = categoriesUsedData
    }
    ...
    var categoriesUsed: Set<String> {
        get {
            guard let data = categoriesUsedData else { return [] }
            return (try? JSONDecoder().decode(Set<String>.self, from: data)) ?? []
        }
        set { categoriesUsedData = try? JSONEncoder().encode(newValue) }
    }
