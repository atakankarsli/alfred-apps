...
struct HomeView: View {
    ...
    private var xpBadge: some View {
        ...
        return HStack(spacing: 14) {
            ZStack {
                Circle()
                    .fill(RadialGradient(
                        colors: [theme.colors.accent, theme.colors.accent.opacity(0.3)],
    ...
    private var playHero: some View {
        let nextLevel = levels.count
        return NavigationLink(value: Route.play(levelIndex: min(nextLevel, 79))) {
            HStack(spacing: 16) {
                ZStack {
                    Circle()
                        .fill(theme.colors.accent.opacity(0.15))
                        .frame(width: 64, height: 64)
                        .scaleEffect(waveAnimate ? 1.15 : 1.0)
                        .blur(radius: 4)
                    Circle()
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("Level \(nextLevel + 1) of 80").font(.system(size: 14, weight: .medium)).secondaryText()
                }
                Spacer()
    ...
    private var modesGrid: some View {
        LazyVGrid(columns: [GridItem(.flexible(), spacing: 12), GridItem(.flexible(), spacing: 12), GridItem(.flexible(), spacing: 12)], spacing: 12) {
            NavigationLink(value: Route.dailyWave) {
                modeCard(icon: "calendar", name: "Daily", color: .cyan)
            }.buttonStyle(BounceButtonStyle())

            NavigationLink(value: Route.sandbox) {
                modeCard(icon: "paintbrush.fill", name: "Sandbox", color: .green)
            }.buttonStyle(BounceButtonStyle())

            NavigationLink(value: Route.worlds) {
                modeCard(icon: "globe.americas.fill", name: "Worlds", color: .purple)
            }.buttonStyle(BounceButtonStyle())
        }
    }
    ...
    private var streakBanner: some View {
        ...
        return HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text(streak > 0 ? "Solve today to keep it going" : "Play daily to build a streak")
        ...
        .background {
            RoundedRectangle(cornerRadius: 16)
                ...
                .overlay {
                    RoundedRectangle(cornerRadius: 16)
                        .strokeBorder(streak > 0 ? Color.orange.opacity(0.2) : theme.colors.cardBorder.opacity(0.08), lineWidth: 1)
                }
        }
    }
    ...
    private var dailyStats: some View {
        HStack(spacing: 0) {
            statItem(value: "\(currentStats?.levelsCompleted ?? 0)", label: "Solved", icon: "water.waves", color: theme.colors.accent)
