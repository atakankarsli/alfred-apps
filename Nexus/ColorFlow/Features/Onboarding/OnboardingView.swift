...
struct OnboardingView: View {
    ...
    private var welcomePage: some View {
        VStack(spacing: 28) {
            ...
            ZStack {
                ...
                Image(systemName: "link.circle.fill").font(.system(size: 64)).foregroundStyle(theme.colors.accent).shadow(color: theme.colors.accent.opacity(0.4), radius: 10)
            }
            Text("Welcome to Nexus").font(.system(size: 30, weight: .black, design: .rounded)).primaryText().multilineTextAlignment(.center)
            Text("Find the hidden connections\nbetween words.").font(.system(size: 18, weight: .medium)).secondaryText().multilineTextAlignment(.center)
            Spacer()
            nextButton
        }
        .padding(32)
    ...
    private var howToPlayPage: some View {
        VStack(spacing: 28) {
            ...
            ZStack {
                ...
                Image(systemName: "square.grid.3x3.fill").font(.system(size: 52)).foregroundStyle(theme.colors.accent)
            }
            Text("How to Play").font(.system(size: 26, weight: .black, design: .rounded)).primaryText()
            VStack(alignment: .leading, spacing: 18) {
                instruction(icon: "1.circle.fill", text: "12 words hide 3 groups of 4")
                instruction(icon: "2.circle.fill", text: "Tap 4 words that share a link")
                instruction(icon: "3.circle.fill", text: "Submit — but watch your mistakes!")
            }
            .padding(.horizontal, 8)
            clusterDemo.padding(.top, 4)
            Spacer()
            nextButton
        }
        .padding(32)
    ...
    private var clusterDemo: some View {
        HStack(spacing: 6) {
            ForEach(["Apple", "Mango", "Grape", "Peach"], id: \.self) { word in
                Text(word)
    ...
    private var readyPage: some View {
        VStack(spacing: 28) {
            ...
            Text("80 puzzles across 5 realms.\nFind every connection.").font(.system(size: 16, weight: .medium)).secondaryText().multilineTextAlignment(.center)
            HStack(spacing: 16) {
                featurePill(icon: "map.fill", text: "5 Realms")
                featurePill(icon: "star.fill", text: "Stars")
                featurePill(icon: "trophy.fill", text: "Badges")
            }
            Spacer()
            GlassButton("Start Linking", icon: "link") {
