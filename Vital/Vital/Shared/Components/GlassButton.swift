...
struct GlassButton: View {
    ...
    var body: some View {
        if #available(iOS 26, *) { Button(action: action) { label }.buttonStyle(.glassProminent) }
        else { Button(action: action) { label.foregroundStyle(fg).background { bg }.clipShape(.capsule) }.buttonStyle(ScaleButtonStyle()) }
    }
