...
struct StatsDetailView: View {
    ...
    var body: some View {
        ZStack {
            ...
            ScrollView {
                VStack(spacing: 16) { levelCard; overviewCard; streakCard; zoneProgress }.padding(16).padding(.bottom, 32)
            }
        }.frame(maxWidth: .infinity, maxHeight: .infinity).themedBackground().navigationTitle("Stats")
    }

    private var levelCard: some View {
        let xp = st?.totalXP ?? 0; let level = TraceConfig.levelForXP(xp); let progress = TraceConfig.xpProgressInLevel(xp); let title = TraceConfig.xpLevelTitle(level)
    ...
    private var overviewCard: some View {
        ...
        return LazyVGrid(columns: cols, spacing: 12) {
            statBox("Puzzles", "\(st?.puzzlesCompleted ?? 0)", "brain.fill", theme.colors.accent)
            statBox("Perfect", "\(st?.threeStarCount ?? 0)", "star.fill", .yellow)
            statBox("Taps", "\(st?.totalMoves ?? 0)", "hand.tap.fill", .green)
    ...
    private var zoneProgress: some View {
        VStack(spacing: 12) {
            Text("Zone Progress").font(.system(size: 17, weight: .bold, design: .rounded)).primaryText().frame(maxWidth: .infinity, alignment: .leading)
            ForEach(MemoryZone.all) { zone in zoneRow(zone) }
        }.padding(16).background { RoundedRectangle(cornerRadius: 18).fill(theme.colors.surface.opacity(0.4)).overlay { RoundedRectangle(cornerRadius: 18).strokeBorder(theme.colors.boardBorder.opacity(0.1), lineWidth: 1) } }
    }

    private func zoneRow(_ zone: MemoryZone) -> some View {
        ...
        return HStack(spacing: 10) {
            Image(systemName: zone.icon).font(.system(size: 16)).foregroundStyle(color).frame(width: 24)
            Text(zone.name).font(.system(size: 14, weight: .bold, design: .rounded)).primaryText()
