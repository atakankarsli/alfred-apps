...
struct HomeView: View {
    ...
    var body: some View {
        ZStack {
            ...
            ScrollView {
                VStack(spacing: 16) {
                    topBar; levelCard; playCard; streakCard; modesRow; navRow; miniStats
    ...
    private var topBar: some View {
        HStack {
            Text("Trace").font(.system(size: 28, weight: .black, design: .rounded)).primaryText()
            Spacer()
    ...
    private var levelCard: some View {
        let xp = st?.totalXP ?? 0; let lvl = TraceConfig.levelForXP(xp); let prog = TraceConfig.xpProgressInLevel(xp); let title = TraceConfig.xpLevelTitle(lvl)
    ...
    private var playCard: some View {
        let next = nextPuzzle
        return NavigationLink(value: Route.trace(mode: .puzzle(index: next))) {
            VStack(spacing: 12) {
                ZStack { Circle().fill(theme.colors.accent.opacity(0.15)).frame(width: 72, height: 72)
                    Image(systemName: "brain.fill").font(.system(size: 32, weight: .bold)).foregroundStyle(theme.colors.accent) }
                Text("Begin Trace").font(.system(size: 20, weight: .black, design: .rounded)).primaryText()
                Text("Puzzle \(next + 1) of \(TraceConfig.totalPuzzles)").font(.system(size: 14, weight: .medium)).secondaryText()
    ...
    private var nextPuzzle: Int {
        ...
        for i in 0..<TraceConfig.totalPuzzles { if best[i] == nil { return i } }
        return 0
    }
    ...
    private var streakCard: some View {
        ...
        return HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text("Practice daily to build a streak").font(.system(size: 12, weight: .medium)).secondaryText()
            }; Spacer()
    ...
    private var modesRow: some View {
        HStack(spacing: 12) {
            NavigationLink(value: Route.trace(mode: .daily)) {
                VStack(spacing: 8) { Image(systemName: "calendar").font(.system(size: 22)).foregroundStyle(theme.colors.accent)
                    Text("Daily").font(.system(size: 13, weight: .bold, design: .rounded)).primaryText()
            ...
            NavigationLink(value: Route.trace(mode: .quickTrace)) {
                VStack(spacing: 8) { Image(systemName: "shuffle").font(.system(size: 22)).foregroundStyle(theme.colors.accent)
                    Text("Quick Trace").font(.system(size: 13, weight: .bold, design: .rounded)).primaryText()
                }.frame(maxWidth: .infinity).padding(.vertical, 18)
                .background { RoundedRectangle(cornerRadius: 16).fill(theme.colors.surface.opacity(0.3)).overlay { RoundedRectangle(cornerRadius: 16).strokeBorder(theme.colors.boardBorder.opacity(0.08), lineWidth: 1) } }
            }.buttonStyle(BounceButtonStyle())
        }
    ...
    private var miniStats: some View {
        HStack(spacing: 0) {
            miniStat("brain.fill", "\(st?.puzzlesCompleted ?? 0)", "Traces")
