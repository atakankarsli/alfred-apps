...
struct OnboardingView: View {
    ...
    private var welcomePage: some View {
        VStack(spacing: 28) {
            ...
            ZStack {
                ...
                Image(systemName: "ear.fill").font(.system(size: 64))
                    .foregroundStyle(theme.colors.accent).shadow(color: theme.colors.accent.opacity(0.4), radius: 10)
            }
            Text("Welcome to Echo").font(.system(size: 30, weight: .black, design: .rounded)).primaryText().multilineTextAlignment(.center)
            Text("Train your ear with\nsound sequences.").font(.system(size: 18, weight: .medium)).secondaryText().multilineTextAlignment(.center)
            Spacer()
            nextButton
        }
        .padding(32)
    ...
    private var mechanicsPage: some View {
        VStack(spacing: 28) {
            ...
            ZStack {
                ...
                Image(systemName: "waveform").font(.system(size: 52)).foregroundStyle(theme.colors.accent)
            }
            Text("How It Works").font(.system(size: 26, weight: .black, design: .rounded)).primaryText()
            VStack(alignment: .leading, spacing: 18) {
                mechInfo(icon: "ear.fill", text: "Listen to the sequence", color: .cyan)
                mechInfo(icon: "hand.tap.fill", text: "Repeat it from memory", color: .green)
                mechInfo(icon: "arrow.up.right", text: "Sequences get longer", color: .orange)
    ...
    private var readyPage: some View {
        VStack(spacing: 28) {
            ...
            Text("5 sonic realms, 80 levels.\nTrust your ears.").font(.system(size: 16, weight: .medium)).secondaryText().multilineTextAlignment(.center)
            HStack(spacing: 16) {
                featurePill(icon: "waveform", text: "5 Realms")
