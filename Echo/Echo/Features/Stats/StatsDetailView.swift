...
struct StatsDetailView: View {
    ...
    private var xpSection: some View {
        ...
        return VStack(spacing: 10) {
            ...
            if level < EchoConfig.xpPerLevel.count {
                Text("\(EchoConfig.xpPerLevel[level] - xp) XP to next level").font(.caption).mutedText()
            }
        }
        .padding(16)
        .background {
    ...
    private var echoStats: some View {
        statsCard(title: "Echoes", rows: [
    ...
    private var streakStats: some View {
        statsCard(title: "Streaks", rows: [
            ...
            ("Daily Echoes", "\(s?.dailyEchoesCompleted ?? 0)"),
    ...
    private var realmStats: some View {
        statsCard(title: "Realms", rows: [
            ("Tones", "\(s?.tonesCompleted ?? 0) levels"),
            ("Melody", "\(s?.melodyCompleted ?? 0) levels"),
            ("Rhythm", "\(s?.rhythmCompleted ?? 0) levels"),
            ("Harmony", "\(s?.harmonyCompleted ?? 0) levels"),
