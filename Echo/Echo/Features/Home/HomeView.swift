...
struct HomeView: View {
    ...
    private var xpBadge: some View {
        ...
        let level = EchoConfig.levelForXP(xp)
        let progress = EchoConfig.xpProgressInLevel(xp)
        let title = EchoConfig.levelTitle(level)
        ...
        return NavigationLink(value: Route.play(levelIndex: nextLevel)) {
            HStack(spacing: 16) {
                ZStack {
                    ...
                    Circle()
                        .fill(RadialGradient(colors: [theme.colors.accent, theme.colors.accent.opacity(0.7)], center: .center, startRadius: 0, endRadius: 28))
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("Level \(nextLevel + 1) of \(EchoConfig.totalLevels)").font(.system(size: 14, weight: .medium)).secondaryText()
                }
                Spacer()
    ...
    private var modesGrid: some View {
        LazyVGrid(columns: [GridItem(.flexible(), spacing: 12), GridItem(.flexible(), spacing: 12), GridItem(.flexible(), spacing: 12)], spacing: 12) {
            NavigationLink(value: Route.dailyEcho) {
                modeCard(icon: "calendar", name: "Daily", color: .cyan)
            }.buttonStyle(BounceButtonStyle())
            NavigationLink(value: Route.endless) {
                modeCard(icon: "infinity", name: "Endless", color: .green)
            }.buttonStyle(BounceButtonStyle())
            NavigationLink(value: Route.worlds) {
                modeCard(icon: "globe.americas.fill", name: "Realms", color: .purple)
            }.buttonStyle(BounceButtonStyle())
        }
    }
    ...
    private var dailyStats: some View {
        HStack(spacing: 0) {
            statItem(value: "\(currentStats?.levelsCompleted ?? 0)", label: "Solved", icon: "ear.fill", color: theme.colors.accent)
