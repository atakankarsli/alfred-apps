...
final class SoundService {
    ...
    func playBreathIn() { play("breath_in") }
    func playBreathOut() { play("breath_out") }
    func playCountdown() { play("tick") }
    func playComplete() { play("success") }
    ...
    private func generateAllSounds() {
        ...
        buffers["breath_in"] = synthesize(duration: 0.5) { t in
            let freq: Float = 200 + 100 * t / 0.5
            let env: Float = t < 0.1 ? t / 0.1 : (0.5 - t) / 0.4
            return sinf(2 * .pi * freq * t) * max(0, env) * 0.15
        }
        buffers["breath_out"] = synthesize(duration: 0.5) { t in
            let freq: Float = 300 - 100 * t / 0.5
            let env: Float = t < 0.05 ? t / 0.05 : (0.5 - t) / 0.45
            return sinf(2 * .pi * freq * t) * max(0, env) * 0.12
        }
    }
