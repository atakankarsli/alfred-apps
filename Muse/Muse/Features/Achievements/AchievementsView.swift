...
struct AchievementsView: View {
    ...
    private var unlockedIds: Set<String> {
        Set(unlocked.map(\.achievementId))
    }
    ...
    private func achievementCard(_ achievement: Achievement) -> some View {
        ...
        return VStack(spacing: 6) {
            ZStack {
                Circle()
                    .fill(isUnlocked ? tierColor(achievement.tier).opacity(0.15) : theme.colors.cardBackground.opacity(0.4))
                    .frame(width: 48, height: 48)
                if showDetails {
                    Image(systemName: achievement.icon)
                        .font(.system(size: 20))
                        .foregroundStyle(isUnlocked ? tierColor(achievement.tier) : theme.colors.textMuted.opacity(0.4))
                } else {
        ...
        .background {
            RoundedRectangle(cornerRadius: 12)
                ...
                .overlay {
                    RoundedRectangle(cornerRadius: 12)
                        .strokeBorder(isUnlocked ? tierColor(achievement.tier).opacity(0.2) : theme.colors.cardBorder.opacity(0.08), lineWidth: 1)
                }
        }
    }
