...
struct SeasonMapView: View {
    ...
    private func seasonCard(_ season: Season) -> some View {
        let completedCount = season.challengeRange.filter { bestStars[$0] != nil }.count
        let starsEarned = season.challengeRange.reduce(0) { $0 + (bestStars[$1] ?? 0) }
        let maxStars = season.challengeCount * 3
        let isLocked = season.firstChallenge > highestUnlocked
        let isComplete = completedCount == season.challengeCount
        ...
        return VStack(spacing: 0) {
            HStack {
                ...
                VStack(alignment: .trailing, spacing: 3) {
                    Text("\(completedCount)/\(season.challengeCount)")
                ...
                GeometryReader { geo in
                    ZStack(alignment: .leading) {
                        ...
                        Rectangle()
                            ...
                            .frame(width: geo.size.width * Double(completedCount) / Double(max(season.challengeCount, 1)), height: 4)
                    }
                }
                .frame(height: 4)
                .padding(.horizontal, 14)

                challengeGrid(season, seasonColor: seasonColor)
    ...
    private func challengeGrid(_ season: Season, seasonColor: Color) -> some View {
        LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 8), count: 5), spacing: 8) {
            ForEach(Array(season.challengeRange), id: \.self) { level in
                let isCompleted = bestStars[level] != nil
                let isAvailable = level <= highestUnlocked
                let localIndex = level - season.firstChallenge + 1

                Button {
                    if isAvailable {
                        appState.navigate(to: .challenge(index: level))
                    }
                } label: {
                    ZStack {
