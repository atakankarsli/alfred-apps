...
struct GlassButton: View {
    ...
    var body: some View {
        if #available(iOS 26, *) { glassButton } else { fallbackButton }
    }
    ...
    private var fallbackButton: some View {
        Button(action: action) {
            label.foregroundStyle(foregroundColor).background { backgroundView }.clipShape(.capsule)
        }
    ...
    private var foregroundColor: Color {
        switch style {
        case .primary: theme.colors.textOnPrimary
        case .secondary: theme.colors.primary
        case .subtle: theme.colors.textSecondary
        }
    }
    ...
    private var backgroundView: some View {
        ...
        case .primary:
            Capsule().fill(theme.colors.primary)
                .overlay { Capsule().fill(LinearGradient(colors: [.white.opacity(0.2), .clear], startPoint: .top, endPoint: .bottom)) }
                .shadow(color: .black.opacity(theme.isDark ? 0.2 : 0.06), radius: 4, y: 2)
        case .secondary:
            Capsule().fill(.ultraThinMaterial)
                .overlay { Capsule().strokeBorder(theme.colors.primary.opacity(0.3), lineWidth: 1) }
        case .subtle:
