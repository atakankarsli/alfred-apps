...
struct HomeView: View {
    ...
    private var currentWorld: SoundWorld { SoundWorld.worldForLevel(nextLevel) }
    ...
    private var playHero: some View {
        NavigationLink(value: Route.drift(mode: .mix(index: nextLevel))) {
            VStack(spacing: 14) {
                ZStack {
                    Circle()
                ...
                VStack(spacing: 4) {
                    heroTitle
                    Text("Mix \(nextLevel + 1) of \(DriftConfig.totalLevels)")
    ...
    private var heroTitle: some View {
        if nextLevel > 0 {
            Text("\(currentWorld.name) — Mix \(SoundWorld.localIndex(forLevel: nextLevel) + 1)")
    ...
    private var streakBanner: some View {
        ...
        return HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text(streak > 0 ? "Mix today to keep it going" : "Mix daily to build a streak")
    ...
    private var modeButtons: some View {
        HStack(spacing: 12) {
            NavigationLink(value: Route.drift(mode: .daily)) {
                modeCard(icon: "calendar", title: "Daily Mix", color: .orange)
            }.buttonStyle(BounceButtonStyle())

            NavigationLink(value: Route.drift(mode: .freeform)) {
                modeCard(icon: "waveform", title: "Free Mix", color: theme.colors.secondary)
            }.buttonStyle(BounceButtonStyle())
        }
    }
    ...
    private var quickNav: some View {
        HStack(spacing: 10) {
            NavigationLink(value: Route.soundscapeMap) {
                gameNavButton(icon: "map.fill", label: "Worlds", color: Color(hex: "6C63FF"))
            }.buttonStyle(BounceButtonStyle())
    ...
    private var statsSection: some View {
        HStack(spacing: 0) {
            gameStat(value: "\(currentStats?.puzzlesCompleted ?? 0)", label: "Mixed", icon: "headphones", color: theme.colors.accent)
            statDivider
