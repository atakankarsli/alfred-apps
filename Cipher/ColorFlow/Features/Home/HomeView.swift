...
struct HomeView: View {
    ...
    private var currentVault: Vault { Vault.vaultForLevel(min(nextLevel, CipherConfig.totalLevels - 1)) }
    ...
    private var xpBadge: some View {
        ...
        return HStack(spacing: 14) {
            xpLevelCircle(level: level)
            xpProgressInfo(rankTitle: rankTitle, xp: xp, progress: progress)
        }
        .padding(16)
        .background {
    ...
    private func xpLevelCircle(level: Int) -> some View {
        ZStack {
            Circle()
    ...
    private func xpProgressInfo(rankTitle: String, xp: Int, progress: Double) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            HStack {
                Text(rankTitle).font(.system(size: 17, weight: .bold, design: .rounded)).primaryText()
                Spacer()
    ...
    private var playHero: some View {
        NavigationLink(value: Route.cipher(mode: .decrypt(index: nextLevel))) {
            VStack(spacing: 14) {
                playButton
                playLabel
            }
    ...
    private var playButton: some View {
        ZStack {
            ...
            Circle()
                ...
                .overlay {
                    Image(systemName: "lock.open.fill").font(.system(size: 28, weight: .bold)).foregroundStyle(.white)
                }
        }
    }
    ...
    private var playLabel: some View {
        VStack(spacing: 4) {
            if nextLevel > 0 {
                Text("\(currentVault.name) — Cipher \(Vault.localIndex(forLevel: nextLevel) + 1)")
                    .font(.system(size: 16, weight: .bold, design: .rounded)).primaryText()
            } else {
                Text("Start Decoding").font(.system(size: 16, weight: .bold, design: .rounded)).primaryText()
            }
            Text("Cipher \(nextLevel + 1) of \(CipherConfig.totalLevels)")
    ...
    private var streakBanner: some View {
        ...
        return HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text(streak > 0 ? "Decode daily to keep it going" : "Decode daily to build a streak").font(.system(size: 12)).secondaryText()
            }
            Spacer()
    ...
    private var modeButtons: some View {
        HStack(spacing: 12) {
            NavigationLink(value: Route.cipher(mode: .daily)) {
                modeCard(icon: "calendar", title: "Daily", color: .orange)
            }.buttonStyle(BounceButtonStyle())
            NavigationLink(value: Route.cipher(mode: .quickCrack)) {
                modeCard(icon: "bolt.fill", title: "Quick Crack", color: theme.colors.secondary)
            }.buttonStyle(BounceButtonStyle())
        }
    }
    ...
    private var quickNav: some View {
        HStack(spacing: 10) {
            NavigationLink(value: Route.vaultMap) {
                navButton(icon: "lock.rectangle.stack.fill", label: "Vaults", color: Color(hex: "6C63FF"))
            }.buttonStyle(BounceButtonStyle())
    ...
    private var statsSection: some View {
        HStack(spacing: 0) {
            gameStat(value: "\(currentStats?.puzzlesCompleted ?? 0)", label: "Decoded", icon: "lock.open.fill", color: theme.colors.accent)
            statDivider
