...
struct GlassButton: View {
    ...
    @available(iOS 26, *) private var glassButton: some View {
        Button(action: action) { label }.buttonStyle(.glassProminent)
    }
    private var fallbackButton: some View {
        Button(action: action) { label.foregroundStyle(foregroundColor).background { backgroundView }.clipShape(.capsule) }
    ...
    @ViewBuilder private var backgroundView: some View {
        switch style {
        case .primary: Capsule().fill(theme.colors.primary)
