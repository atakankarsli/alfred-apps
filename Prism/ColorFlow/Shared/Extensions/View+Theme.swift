...
private struct MutedTextModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content.foregroundStyle(theme.colors.textMuted)
    }
}

extension Font {
    static func number(_ size: CGFloat, font: NumberFont) -> Font {
        .system(size: size, design: font.fontDesign)
    }
}
