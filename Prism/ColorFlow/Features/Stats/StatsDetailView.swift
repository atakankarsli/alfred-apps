...
struct StatsDetailView: View {
    ...
    private var xpSection: some View {
        ...
        return VStack(spacing: 10) {
            ...
            if level < PrismConfig.xpPerLevel.count {
                Text("\(PrismConfig.xpPerLevel[level] - xp) XP to next level").font(.caption).mutedText()
            }
        }
        .padding(16)
        .background {
    ...
    private func spectrumsClearedCount() -> Int {
        let best = s?.bestMoves ?? [:]
        return Spectrum.all.filter { spectrum in
            spectrum.levelRange.allSatisfy { best[$0] != nil }
        }.count
    }
}
