...
struct SettingsView: View {
        ...
        Section("Appearance") {
            NavigationLink(value: Route.themePicker) {
                HStack {
                    Text("Theme")
                    Spacer()
                    Text(Themes.theme(for: currentSettings?.currentTheme ?? "dawn").name)
    ...
    private var showTimerBinding: Binding<Bool> {
        Binding(
            get: { currentSettings?.showTimer ?? true },
            set: {
                currentSettings?.showTimer = $0
    ...
    private var soundBinding: Binding<Bool> {
        Binding(
            ...
            set: {
                currentSettings?.soundEffects = $0
                SoundService.shared.configure(enabled: $0)
                HapticsService.selection()
            }
        )
    }
    ...
    private var hapticsBinding: Binding<Bool> {
        Binding(
            ...
            set: {
                currentSettings?.haptics = $0
                HapticsService.configure(enabled: $0)
                HapticsService.selection()
            }
        )
    }
                    ...
                    Task {
                        ...
                        if granted {
                            NotificationService.scheduleDailyReminder(
                                hour: currentSettings?.dailyReminderHour ?? 19,
