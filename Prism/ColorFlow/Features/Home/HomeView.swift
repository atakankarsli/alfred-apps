...
struct HomeView: View {
    ...
    private var currentSpectrum: Spectrum { Spectrum.spectrumForLevel(nextLevel) }
    ...
    private var xpBadge: some View {
        ...
        let level = PrismConfig.levelForXP(xp)
        let progress = PrismConfig.xpProgressInLevel(xp)
        let title = PrismConfig.xpLevelTitle(level)
    ...
    private var playHero: some View {
        NavigationLink(value: Route.prism(mode: .level(index: nextLevel))) {
            VStack(spacing: 14) {
                playHeroIcon
                playHeroLabel
            }
    ...
    private var playHeroIcon: some View {
        ZStack {
            Circle()
    ...
    private var playHeroLabel: some View {
        VStack(spacing: 4) {
            if nextLevel > 0 {
                Text("\(currentSpectrum.name) — Level \(Spectrum.localIndex(forLevel: nextLevel) + 1)")
                    .font(.system(size: 16, weight: .bold, design: .rounded)).primaryText()
            } else {
                Text("Refract Light").font(.system(size: 16, weight: .bold, design: .rounded)).primaryText()
            }
            Text("Level \(nextLevel + 1) of \(PrismConfig.totalLevels)")
    ...
    private var streakBanner: some View {
        ...
        return HStack(spacing: 10) {
            ...
            Spacer()
            if streak >= 7 {
                Image(systemName: "shield.checkered").font(.system(size: 18)).foregroundStyle(.orange)
            }
        }
        .padding(14)
        .background {
            RoundedRectangle(cornerRadius: 16)
                ...
                .overlay {
                    RoundedRectangle(cornerRadius: 16)
                        .strokeBorder(streak > 0 ? Color.orange.opacity(0.2) : theme.colors.boardBorder.opacity(0.08), lineWidth: 1)
                }
        }
    }
    ...
    private var modeButtons: some View {
        HStack(spacing: 12) {
            NavigationLink(value: Route.prism(mode: .daily)) {
                modeCard(icon: "calendar", title: "Daily", color: .orange)
            }.buttonStyle(BounceButtonStyle())
            NavigationLink(value: Route.prism(mode: .freePlay)) {
                modeCard(icon: "infinity", title: "Free Play", color: theme.colors.secondary)
            }.buttonStyle(BounceButtonStyle())
        }
    }
    ...
    private var quickNav: some View {
        HStack(spacing: 10) {
            NavigationLink(value: Route.spectrumMap) {
                navButton(icon: "map.fill", label: "Spectrums", color: Color(hex: "6C63FF"))
            }.buttonStyle(BounceButtonStyle())
    ...
    private var statsSection: some View {
        HStack(spacing: 0) {
            gameStat(value: "\(currentStats?.puzzlesCompleted ?? 0)", label: "Solved", icon: "triangle.fill", color: theme.colors.accent)
            statDivider
