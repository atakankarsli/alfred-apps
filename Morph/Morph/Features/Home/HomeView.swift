...
struct HomeView: View {
    ...
    var body: some View {
        ...
        .toolbar {
            ToolbarItem(placement: .principal) {
                Text("Morph").font(.system(size: 28, weight: .black, design: .rounded)).primaryText()
            }
            ToolbarItem(placement: .topBarTrailing) {
                NavigationLink(value: Route.settings) {
                    Image(systemName: "gearshape.fill").font(.system(size: 18))
                        .foregroundStyle(theme.colors.textSecondary).frame(width: 36, height: 36)
                }
            }
        }
        .onAppear {
            withAnimation(.easeInOut(duration: 1.8).repeatForever(autoreverses: true)) { pulseAnimate = true }
        }
    }
    ...
    private var xpBadge: some View {
        ...
        return HStack(spacing: 14) {
            ZStack {
                Circle().fill(RadialGradient(colors: [theme.colors.accent, theme.colors.accent.opacity(0.3)], center: .center, startRadius: 0, endRadius: 28))
                    .frame(width: 56, height: 56).shadow(color: theme.colors.accent.opacity(0.4), radius: 8)
    ...
    private var playHero: some View {
        ...
        let nextLevel = (0..<80).first { !completedIndices.contains($0) } ?? 79
        return NavigationLink(value: Route.play(levelIndex: nextLevel)) {
            HStack(spacing: 16) {
                ZStack {
                    Circle().fill(theme.colors.accent.opacity(0.15)).frame(width: 64, height: 64)
                        .scaleEffect(pulseAnimate ? 1.15 : 1.0).blur(radius: 4)
                    Circle().fill(RadialGradient(colors: [theme.colors.accent, theme.colors.accent.opacity(0.7)], center: .center, startRadius: 0, endRadius: 28))
                        .frame(width: 56, height: 56).shadow(color: theme.colors.accent.opacity(0.5), radius: 8)
                    Image(systemName: "play.fill").font(.system(size: 24, weight: .bold)).foregroundStyle(.white)
                }
                VStack(alignment: .leading, spacing: 4) {
                    Text("Continue").font(.system(size: 20, weight: .black, design: .rounded)).primaryText()
                    Text("Level \(nextLevel + 1) of \(MorphConfig.totalLevels)").font(.system(size: 14, weight: .medium)).secondaryText()
                }
                Spacer()
                Image(systemName: "chevron.right").font(.system(size: 16, weight: .bold)).secondaryText()
            }
            .padding(16)
            .background {
                RoundedRectangle(cornerRadius: 20).fill(theme.colors.accent.opacity(0.06))
                    .overlay { RoundedRectangle(cornerRadius: 20).strokeBorder(theme.colors.accent.opacity(0.15), lineWidth: 1) }
            }
        }
    ...
    private var modesGrid: some View {
        LazyVGrid(columns: [GridItem(.flexible(), spacing: 12), GridItem(.flexible(), spacing: 12), GridItem(.flexible(), spacing: 12)], spacing: 12) {
            NavigationLink(value: Route.dailyMorph) { modeCard(icon: "calendar", name: "Daily", color: .cyan) }.buttonStyle(BounceButtonStyle())
            NavigationLink(value: Route.endless) { modeCard(icon: "infinity", name: "Endless", color: .green) }.buttonStyle(BounceButtonStyle())
            NavigationLink(value: Route.worlds) { modeCard(icon: "globe.americas.fill", name: "Realms", color: .purple) }.buttonStyle(BounceButtonStyle())
        }
    }

    private func modeCard(icon: String, name: String, color: Color) -> some View {
        VStack(spacing: 8) {
            ZStack {
            ...
            Text(name).font(.system(size: 12, weight: .bold, design: .rounded)).primaryText()
        }
    ...
    private var streakBanner: some View {
        ...
        return HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text(streak > 0 ? "Play today to keep it going" : "Play daily to build a streak")
        ...
        .padding(14)
        .background {
            RoundedRectangle(cornerRadius: 16).fill(streak > 0 ? Color.orange.opacity(0.06) : theme.colors.surface.opacity(0.3))
    ...
    private var dailyStats: some View {
        HStack(spacing: 0) {
            statItem(value: "\(currentStats?.levelsCompleted ?? 0)", label: "Solved", icon: "checkmark.circle.fill", color: theme.colors.accent)
